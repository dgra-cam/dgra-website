---
title: Community
description: Community channels for Darwin Green, Franklin Gardens, and King's Court
weight: 10
---

The residents, the users, and the friends of Darwin Green, Franklin Gardens,
and King's Court and their facilities form the Darwin Green community.

## Online Presence

You found **the website for the residents' association** already,
congratulations! We're also present on social media:

- <i class='fa-brands fa-facebook-square'></i> [DGRA Facebook page][fb-dgra]
- <i class='fa-brands fa-instagram'></i> [DGRA on Instagram][ig-dgra]
- <i class='fa-brands fa-x-twitter'></i> [DGRA on X/Twitter][x-dgra]

The community is also active on **Facebook group <i class='fa-brands
fa-facebook'></i> [Darwin Green Plaza][fb-plaza]**.

### WhatsApp

There is also a **community chat** on **<i class='fa-brands fa-whatsapp'></i>
WhatsApp**, available to residents on invitation. If you live in or around
Darwin Green and would like to request an invitation, please **[contact
us][contact]**. Here's a brief reminder of the chat's code of conduct:

> - Be polite and kind
> - Do not send spam
> - Use first names only
> - Do not swear; keep the chat family-friendly
> - Never (ever!) talk about politics
> - There is zero tolerance to aggression or anti-social behaviour in the chat or
>   in Darwin Green, Franklin Gardens, or King's Court; administrators may
>   enforce moderation by removing users from the chat
> - Stick to English, the common language of the chat

There are several hundred members in the chat, so treat it as public space when
you post information.

[contact]: /contact/
[fb-plaza]: https://www.facebook.com/groups/dgplaza/
[fb-dgra]: https://www.facebook.com/DarwinGreenCambridge/
[ig-dgra]: https://www.instagram.com/darwingreencambridge
[x-dgra]: https://x.com/darwingreencam

## Acronyms

Here are some acronyms to understand what the community talks about:

BDW
: Barratt / David Wilson (now Barratt Redrow)

BR
: Barratt Redrow

BSG
: Buy/Sell/Gift chat group

DGMC
: Darwin Green Management Company, a RMC managed by residents and taking care
  of the [HANDLE][map-names] blocks as well as Barratt's sales office.

FN
: Friendly Neighbours chat group

RMC
: Resident Management Company

See also the list of [acronyms and short names][map-names] for places in Darwin
Green's area.

[map-names]: /docs/lore/map/#acronyms-and-local-names
